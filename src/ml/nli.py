...
class NLI_Finetune:
    ...
    def __post_init__(self):
        def split_dataset():
            ...
            """
            bbcids: Set[str] = set(self.dataset["train"]["bbcid"])
            ...
            # filter the dataset
            self.dataset["test"] = self.dataset["train"].filter(
                lambda example: example["bbcid"] in test_bbcids
            )
            self.dataset["train"] = self.dataset["train"].filter(
                lambda example: example["bbcid"] in train_bbcids
            )

            # make sure that there don't exist a bbcid in both train and test
            train_bbcids = set(self.dataset["train"]["bbcid"])
            test_bbcids = set(self.dataset["test"]["bbcid"])
            assert (
                len(train_bbcids.intersection(test_bbcids)) == 0
            ), "The same bbcid exists in both train and test"
        ...
        self.dataset = self.dataset.filter(lambda example: example["is_factual"] != -1)
